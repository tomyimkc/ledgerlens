...
| E-07 | A controlled DataHub OSS document write and official-MCP retrieval occurred. | Inspect the presentation-safe write-back receipt and its source-receipt digest. | `benchmarks/incident_commander/public-datahub-live-writeback-receipt.json` | Redacted derivative of the archived source receipt; local DataHub OSS v1.6.0 evidence. It does not prove recovery, causality, or a hosted public DataHub deployment. | Meaningful DataHub use and write-back |
| E-08 | A planner and two verifier variants produced a bounded, policy-authorized rehearsal plan. | Inspect the presentation-safe rehearsal receipt and its source-receipt digest. | `benchmarks/incident_commander/public-ai-verification-receipt.json` | Redacted derivative of the archived source receipt. No provider action occurred; model labels do not establish provider-family independence. | Technical execution; originality |
...
| E-21 | The final hosted contract can be sampled repeatedly instead of relying on one screenshot or one request. | Open the latest **Hosted evidence continuity** workflow and download its sanitized receipt, or run `make hosted-continuity`. | `.github/workflows/hosted-continuity.yml`, `scripts/check_hosted_continuity.py`, `benchmarks/incident_commander/live-evidence-ladder.json` | Short-window, credential-free samples of the public fixture, Seal Lab refusal, and Context Cut decisions. No provider tool executes. This is not an uptime SLO, provider-reliability study, production validation, or independent review. | Technical execution; submission quality |
...
4. Open **Live Evidence Ladder**. Verify that E-16 provider execution, E-07 DataHub write/read,
   and E-21 repeated hosted checks stay separate, and that the missing same-run proof is explicit.
5. Trigger one replay and verify: DataHub context → bounded plan → verifier review → deterministic
   policy → synthetic receipts → write-back → next-agent handoff.
6. Confirm that every public replay receipt is `fixture://` and that cause, impact, and recovery
   remain unknown.
7. Use E-02 and E-07 to inspect the separate DataHub context and write-back evidence.
8. Use E-16 for the separate one-shot four-provider rehearsal; do not generalize it into sustained
   operation.
9. Inspect E-21 for repeated public contract samples; do not generalize them into provider
   reliability or an uptime SLO.
10. Run `make judge-check` for the primary local verification path.
