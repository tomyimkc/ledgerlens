...
| E-16 | A single run can execute the whole chain — plan, verify, authorize, and one bounded action against all four providers — into one linked receipt. | Inspect the offline-tested script and its fanout tests; run `--confirm-live` with owner credentials to emit the receipt. | `scripts/run_live_incident_rehearsal.py`, `tests/test_live_incident_rehearsal.py`, `docs/LIVE_PROVIDER_REHEARSAL.md` | Infrastructure ready and offline-tested; the live receipt is produced only on a supervised run with the owner's throwaway credentials. Each provider action is one bounded rehearsal, not sustained reliability; no causality or recovery is established. | Technical execution; real-world usefulness |
