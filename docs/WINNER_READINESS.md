...
**Scope:** Public repository, hosted fixture, recorded receipts, regenerated local video cut, and
reproducibility.
...
I/O page, interactive server-evaluated Seal Lab, E-20 DataHub Context Cut, the Live Evidence
Ladder, and the credential-free E-21 continuity sampler. It remains conservative because the
public incident state is a fixture, E-21 executes no provider tool, and no production outcome or
...
|---|---:|---|---|---|---|---|
| A. Meaningful DataHub use and write-back | 7.5 / 10 | Medium | Context flow; **tool-specific DataHub evidence contracts + public Context Cut (E-20)**; real-pipeline gate ablation (E-15); local DataHub OSS write-back + official-MCP retrieval (E-07); Agent Context Kit framing | The public Space still does not contact DataHub; E-20 is a synthetic controlled ablation over a recorded plan | Use the supervised integrated harness only after owner review of the synthetic context seed and live destinations | Finalist-capable |
| B. Technical execution and end-to-end functionality | 7.8 / 10 | High | Typed orchestrator; plan-exact gate; per-tool fact contracts; abstention support; deterministic tests; E-15; **one-run live fanout across all four providers (E-16)**; CI; repeated public contract sampling (E-21); fail-closed combined-run harness | A single bounded rehearsal per provider; E-21 checks fixture/policy surfaces only; the combined harness has not produced a successful receipt | Complete one owner-authorized DataHub read→sealed actions→write-back→exact-URN read-back run, or keep the gap open | Finalist-capable |
...
| E. Submission quality and reproducibility | 7.8 / 10 | High | Public Apache-2.0 repo/Space/video; Seal Lab; Context Cut; Live Evidence Ladder; recorded Agent I/O; examples; one-command rebuild; CI; secret scan; evidence index; clean-clone receipt | The public YouTube title still uses stale “Autonomous” wording; playback needs a final incognito check; final release/package alignment is time-sensitive | Rename the video, deploy the final revision, run E-21 against that revision, verify public URLs in a clean browser, and record the release SHA | Finalist-capable |
...
- E-21 replaces a single hosted screenshot/request with time-separated public contract samples,
  but it deliberately executes no provider tool and is not an uptime or provider-reliability study.
...
- A regenerated 150.059-second Policy-Sealed cut passed codec, duration, caption-stream, digest,
  and representative-frame checks. It labels the generated presenter throughout and uses captured
  product/evidence panels for functionality. The public YouTube title still says “Autonomous,” so
  the replacement cut still requires an account-authorized upload or rename plus a final
  clean/incognito-browser check.
...
| P0 | Public demo shows no DataHub involvement. | A | The one clickable artifact never touches DataHub. | Use `docs/INTEGRATED_LIVE_REHEARSAL.md` for one supervised owner-controlled run; never put its credentials on the Space. | One same-process receipt with successful exact-URN read-back. | Harness implemented; owner authorization pending |
...
| P0 | Public video title still says “Autonomous.” | C, E | It contradicts the product's authority-outside-the-model thesis in the first judge-visible label. | Upload the verified replacement cut or rename the existing video to **LedgerLens — Policy-Sealed Incident Commander (DataHub Agent Hackathon)**. | YouTube watch page and Devpost embed show the new title and the replacement playback passes incognito review. | Replacement cut ready; account-authorized publish action pending |
...
| P1 | Live evidence was easy to flatten into one implied production run. | A, B, E | Separate bounded events can look stronger than they are when scattered across receipts. | Publish the Live Evidence Ladder, digest-bind E-16/E-07, disclose separate-process status, and add E-21 repeated public samples. | `live-evidence-ladder.json`, public UI, continuity workflow receipt. | **Implemented; final workflow run pending** |
...
8. **Live evidence without theater:** Open the Live Evidence Ladder. Confirm E-16 and E-07 are
   digest-bound, share an incident ID, and are explicitly marked as separate runs; inspect E-21
   as a no-provider-tool continuity sample.
9. **Reproduce:** Run `make judge-check`; it rebuilds E-20, the public receipt derivatives, and
   the evidence ladder without credentials before the gates.
