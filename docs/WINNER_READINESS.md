...
| B. Technical execution and end-to-end functionality | 7.0 / 10 | High | Typed orchestrator; deterministic authority tests; real-pipeline `PolicyGate`/`VerifierPanel` benchmark; four adapters; **offline-tested one-run provider fanout (E-16)**; CI; hosted smoke | Only GitHub has a *produced* live provider receipt; the four-provider live receipt is ready but not yet run | Owner runs `run_live_incident_rehearsal.py --confirm-live` to emit the linked receipt | Finalist-capable |
...
- The strongest *produced* live provider evidence is still one closed GitHub rehearsal. The four-provider live rehearsal is built and offline-tested but needs the owner's throwaway credentials to run.
...
| P0 | Only GitHub has a produced live provider receipt. | B, D | "Agents that do real work" wants demonstrated action. | Owner runs the offline-tested one-run rehearsal with scoped credentials. | `docs/LIVE_PROVIDER_REHEARSAL.md`; E-16 receipt. | **Infra done (PR #25)**; owner runs |
